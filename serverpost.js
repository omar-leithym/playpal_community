...
const protect = asyncHandler(async (req, res, next) => {
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
        try {
            ...
            const [rows] = await connection.query('SELECT * FROM user WHERE id = ?', [decoded.id]);
            connection.release();
...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        const [userExists] = await connection.query('SELECT * FROM user WHERE username = ?', [username]);
        const [emailExists] = await connection.query('SELECT * FROM user WHERE email = ?', [email]);
...
app.get("/profile", async (req, res) => {
    ...
    try {
        ...
        const [userRows] = await connection.query('SELECT username, bio,firstName FROM user WHERE email = ?', [email]);
        ...
        const [otherRows] = await connection.query('SELECT urlPic FROM userPics WHERE email = ?', [email]);

        const urlPics = otherRows.map(row => row.urlPic); // Extracting urlPics from rows
       // console.log(urlPics);
        //console.log(otherRows);
        const [otherRows2] = await connection.query('SELECT P.sport_name,P.skillLevel FROM Plays P INNER JOIN user u ON  u.email=P.email WHERE u.email= ?', [email]);
        ...
        res.status(200).json({ firstName,username, bio, urlPics,favSports,favSportsSkill }); //now add favSports    favSports and favSportsSkill are dependent on each other each index represents one object
...
app.get("/pic", async (req, res) => {
    ...
    try {
        const connection = await pool.getConnection();
        const [rows] = await connection.query('SELECT profilePicURL FROM user WHERE email= ?', [email]);
        connection.release();
        //console.log(rows[0]);
        if (rows.length === 0) {
            return res.status(404).json({ message: "User not found" });
...
app.get("/bio", async (req, res) => {
  ...
  //  console.log(email);
    try {
        const connection = await pool.getConnection();
        const [rows] = await connection.query('SELECT bio from user WHERE email= ?', [email]);
...
app.get('/pending-requests/:userId', asyncHandler(async (req, res) => {
...
app.post("/addfriend", async (req, res) => {
    ...
    try {
        const connection = await pool.getConnection();
        const [result] = await connection.query('INSERT INTO friend(user_1, user_2) VALUES (?, ?);', [email, email2]);
        connection.release();
    ...
    } catch (error) { //if it is an error then they're already friends technically but this could lead to a bug later for example if the database is down
...
app.delete('/delete-match-request', async (req, res) => {
    ...
    if (!requester_id || !requested_id) {
        return res.status(400).json("Please provide both requester_id and requested_id");
    }
...
app.post('/tournament/signup', async (req, res) => {
    ...
    if (!tournament_id || !user_id || !team_name) {
        return res.status(400).json("Please provide tournament_id, user_id, and team_name");
    }
    ...
    try {
        ...
        if (existingRegistration.length > 0) {
            connection.release();
            return res.status(400).json("User already registered for this tournament");
        }
...
app.post('/tournament/withdraw', async (req, res) => {
    ...
    if (!tournament_id || !user_id) {
        return res.status(400).json("Please provide both tournament_id and user_id");
    }
...
app.post('/changePassword', async (req, res) => {
    ...
    try {
        ...
        const connection = await pool.getConnection();console.log("ab to run query");
        ...
        if (isMatch) {
            ...
            await updateConnection.query('UPDATE user SET password = ? WHERE email = ?', [hashedNewPassword, email]);
...
app.post('/updatelocation', async (req, res) => {
    ...
    try {
        const connection = await pool.getConnection();
        const [result] = await connection.query('UPDATE user SET location = ? WHERE email = ?', [location, email]);
        connection.release();
        console.log(result.affectedRows)
        if (result.affectedRows > 0) {
            res.status(200).json({ message: 'Location updated successfully' });
        } else {
            res.status(404).json({ message: 'User not found or Location not updated' });
        }
    } catch (error) {
...
app.post('/updatebio', async (req, res) => {
    ...
    try {
        const connection = await pool.getConnection();
        const [result] = await connection.query('UPDATE user SET bio = ? WHERE email = ?', [bio, email]);
        connection.release();
...
app.get('/api/friends', async (req, res) => {
    ...
    try {
        ...
        const [results] = await connection.query(`
            ...
            FROM friend AS f
            JOIN user AS u ON f.user_2 = u.email
            WHERE f.user_1 = ?
        ...
        ORDER BY c.timestamp DESC LIMIT 1) AS last_message
        FROM friend AS f
        JOIN user AS u ON f.user_1 = u.email	
        WHERE f.user_2 = ?
...
app.get('/api/user', async (req, res) => {
    ...
    try {
        const connection = await pool.getConnection();
        const [results] = await connection.query('SELECT username AS name, profilePicURL AS profilePicUrl FROM user WHERE email = ?', [userEmail]);
        connection.release();
...
app.post('/api/chats', async (req, res) => {
    ...
    try {
        const connection = await pool.getConnection();
        const [result] = await connection.query('INSERT INTO chat (sender_email, receiver_email, message) VALUES (?, ?, ?)', [sender_email, receiver_email, message]);
        connection.release();
...
app.get('/tournaments/my', async (req, res) => {
  ...
  app.post('/uploadPicture', async (req, res) => {
    ...
    console.log(email);
    if (!email || !urlPic) {
        return res.status(400).json({ success: false, message: 'Email and Picture URL are required.' });
    }
    ...
    try {
        ...
        res.status(200).json({ success: true, message: 'Picture URL uploaded successfully.' });
    } catch (err) {
...
app.post('/updateprofilepic', async (req, res) => {
    ...
    try {
        const connection = await pool.getConnection();
        const [result] = await connection.query('UPDATE user SET profilePicURL = ? WHERE email = ?', [profilePicURL, email]);
        connection.release();

        if (result.affectedRows > 0) {
            res.status(200).json({ message: 'Profile picture URL updated successfully' });
        } else {
            res.status(404).json({ message: 'User not found or profile picture URL not updated' });
        }
    } catch (error) {
        console.error("Error updating profile picture URL:", error);
        res.status(500).json({ message: 'Server error' });
